...
每次推送还会维护仓库根目录的 `index.html` 工具菜单：不存在时创建独立的简洁首页；已存在时只添加或更新带 `tools-platform:menu` 标记的菜单片段，不改写原有页面其他内容。同时维护仓库说明：优先更新已有的 `repository-guide.md`，否则更新已有的 `README.md`；都没有时创建 `README.md`。说明文件的工具介绍使用 `tools-platform:guide` 标记管理，其他人工编写内容保留。根目录 `.tools-platform-menu.json` 按工具 slug 保存入口与简介，便于后续工具扩展。根目录 `index.html` 因此保留为站点首页，不能再配置为具体工具的 HTML 路径；若已有这样的配置，请改为 `{toolSlug}/index.html`。

服务端每次从远端克隆目标分支最新内容，仅暂存本工具已变化的发布文件、首页、菜单清单与仓库说明，再提交并推送；事件更新时通常仅变更 `records.json` 与 `audit.json`，未变化的首页、说明、HTML、JSON 和附件不会重复提交。已删除的证据会从本工具的 `data/evidence/` 清理；在同一 HTML 路径下从分离版切回单文件时清理本工具生成的 JSON 和证据文件，不清空仓库其他代码。更改 HTML 路径不会自动删除旧路径，需人工检查并清理旧发布文件。可选择启用每 5–1440 分钟自动推送，后台启动后按租户调度，无需打开工具页面；数据未变化时不重复提交。定时任务仅在服务端进程运行时执行：绿色版 EXE 关闭窗口但仍在托盘运行时会继续，完全退出程序后停止。Windows 绿色版未内置 Git，运行电脑须安装 `git.exe` 并使其在 `PATH` 中，且预先配置 CodeHub Git 凭据；设置页提供 Git 检查及安装链接，CodeHub 认证可参照其页面右上角帮助中心。不要在远端地址里填写密码或 Token。快照包含当前租户的人员、事件、审计和证据附件，发布前请确认仓库及 Pages 的可见范围；外部证据链接仍依赖原站点。只读页面中的按钮保留，但写入操作会提示联系管理员；该提示不构成数据保密或访问控制，访问权限应由静态托管平台配置。
