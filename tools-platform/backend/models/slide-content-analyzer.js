...
async function buildDeckTaxonomy(client, slides, settings) {
...
${JSON.stringify(outline)}`;
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            if (unique.length >= 5) return { deckTopic: normalizeText(parsed.deckTopic).slice(0, 100), categories: unique.slice(0, 10) };
        } catch (error) {
...
async function analyzeChunk(client, chunk, taxonomy, settings, retryNote = '') {
...
async function refineSummaries(client, slides, items, settings) {
...
async function analyzeSlides(slides) {
    ...
    try {
        ...
        if (!settings.hasApiKey || !settings.keyLooksValid) return { items: fallback, usedAi: false, taxonomy: defaultTaxonomy() };
        const client = aiProviderClient.createClient(settings);
        const taxonomy = await buildDeckTaxonomy(client, slides, settings);
        ...
        for (let offset = 0; offset < contexts.length; offset += 5) {
            ...
            try {
                analyzed.push(...await analyzeChunk(client, chunk, taxonomy, settings));
            } catch (firstError) {
                try {
                    analyzed.push(...await analyzeChunk(client, chunk, taxonomy, settings, '\n上次输出被截断，请进一步缩短 summary 和 intent，确保 JSON 完整闭合。'));
                } catch (secondError) {
                    for (const single of chunk) {
                        try {
                            analyzed.push(...await analyzeChunk(client, [single], taxonomy, settings, '\n这是逐页修复请求，请确保 JSON 完整闭合。'));
                        } catch (singleError) {
                            warnings.push(`第${single.pageNumber}页：${singleError.message}`);
                        }
                    }
                    console.warn(`[slide-design] AI chunk degraded (${chunk[0].pageNumber}-${chunk[chunk.length - 1].pageNumber}):`, secondError.message || firstError.message);
                }
            }
        }
        ...
        const refinedItems = await refineSummaries(client, slides, normalizedItems, settings);
        return {
