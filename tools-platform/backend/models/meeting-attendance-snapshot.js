...
const { safeJson, collectMeetingData, staticRuntimeSource } = require('./static-snapshot-data');

const TOOL_SLUG = 'tool-msf5b7nn';
...
async function loadSource() {
    ...
    if (!sourcePath) throw new Error('会议考勤工具尚未安装');
    const html = fs.readFileSync(sourcePath, 'utf8');
    if (!html.includes("fetch('/api/meeting-snapshots')")) throw new Error('会议考勤模板与静态快照适配器不兼容');
    return html;
}
