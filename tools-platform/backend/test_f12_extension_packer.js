...
async function testChromeCaptureTemplateCompatibility() {
    ...
    assert.ok(opened.file('capture-background.js'), '模板包必须包含后台错误桥接入口');
    assert.ok(opened.file('capture-error-background.js'), '模板包必须包含后台友好报错处理');
    assert.ok(opened.file('capture-error-ui.js'), '模板包必须包含 Popup 友好报错界面');
    ...
    const templateManifest = JSON.parse(await opened.file('manifest.json').async('string'));
    assert.strictEqual(templateManifest.background.service_worker, 'capture-background.js', '后台必须先加载错误桥接再运行原始 bundle');
    const captureBackground = fs.readFileSync(path.join(toolDir, 'chrome-capture-pro/capture-background.js'), 'utf8');
    const captureErrorBackground = fs.readFileSync(path.join(toolDir, 'chrome-capture-pro/capture-error-background.js'), 'utf8');
    const captureErrorUi = fs.readFileSync(path.join(toolDir, 'chrome-capture-pro/capture-error-ui.js'), 'utf8');
    assert.ok(captureBackground.includes("import './capture-error-background.js'"), '必须先注册后台错误处理');
    assert.ok(captureBackground.includes("import './dist/background/bundle.js'"), '必须继续运行原始截图后台');
    assert.ok(captureErrorBackground.includes("addEventListener('unhandledrejection'"), '必须捕获截图 Promise 异常');
    assert.ok(captureErrorBackground.includes("userVisible: false"), '页面探测无接收端时不得误报截图失败');
    assert.ok(captureErrorBackground.includes('continuing with dynamic injection'), '必须将消息探测失败视为动态注入回退');
    assert.ok(captureErrorBackground.includes('function beginCaptureAttempt'), '新的截图操作必须清除上一次错误状态');
    assert.ok(captureErrorBackground.includes("chrome.action.setBadgeText({ text: '!' })"), '限制页无法注入时必须在扩展图标提示失败');
    assert.ok(captureErrorBackground.includes('chrome.scripting.executeScript'), '普通网页必须直接显示友好错误');
    assert.ok(captureErrorUi.includes('chrome-capture-popup-error'), 'Popup 必须显示已保存的截图错误');
