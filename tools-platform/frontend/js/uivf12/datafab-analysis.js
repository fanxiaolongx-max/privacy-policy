...
(function () {
    ...
    function installDataFabAnalysisRuntime(root, options) {
        ...
        if (!root || (!simulationMode && window.location.hostname !== 'datafab-pro.gtsdata.huawei.com')) return null;
        ...
        let active = false; let loading = false; let destroyed = false; let loadToken = 0; let dashboardData = null; let activeTopic = 'return'; let detailPage = 1; let detailPageSize = 50; let capturedAt = null; let reviewInfo = null;
        function tr(zh, en) { return settings.language === 'en' ? en : zh; }
        function saveSettings() { if (reviewInfo) return; try { localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings)); } catch (error) {} }
        ...
        function planStartValue(row) {
            return row && row.planStart !== undefined ? row.planStart : rowField(row, ['计划开始时间', '计划开始日期', '计划启动时间', 'plan_start_time', 'planStartTime', 'plan_start_date', 'start_time']);
        }
        function planStartTimestamp(value) {
        ...
        function isRecentPlanStart(value) {
            const timestamp = planStartTimestamp(value); if (!Number.isFinite(timestamp)) return false;
            const today = new Date(); today.setHours(23, 59, 59, 999);
            const earliest = new Date(today); earliest.setDate(earliest.getDate() - 6); earliest.setHours(0, 0, 0, 0);
            return timestamp >= earliest.getTime() && timestamp <= today.getTime();
        }
        function sortByPlanStart(rows) {
            return (rows || []).slice().sort((a, b) => {
                const difference = planStartTimestamp(planStartValue(a)) - planStartTimestamp(planStartValue(b));
                return difference || 0;
            });
        }
        ...
        function filingInsightsSection(rows) {
            const insight = analyzeFilingRows(rows); const topBu = insight.dimensions.bu[0]; const topCustomer = insight.dimensions.customer[0]; const topOperator = insight.operators[0]; const reasonMax = Math.max(1, ...insight.reasons.map(item => item.count)); const explained = sortByPlanStart(insight.detail.filter(row => row.hasReason || row.hasRemark));
            ...
            const detail = '<section class="df-card"><header><div><strong>' + tr('备案原因与备注明细', 'Filing Reasons & Notes') + '</strong><small>' + tr('按计划开始时间由近到远；最近 7 天内计划行已高亮', 'Sorted by planned start time; rows planned within the last 7 days are highlighted') + '</small></div><span class="df-count">' + fmt(explained.length) + tr(' 条', ' rows') + '</span></header><div class="df-table-wrap df-filing-detail"><table><thead><tr><th>' + tr('任务单号', 'Task No.') + '</th><th>' + tr('操作人', 'Operator') + '</th><th>BU</th><th>' + tr('客户名', 'Customer') + '</th><th>' + tr('产品族', 'Product Family') + '</th><th>' + tr('产品', 'Product') + '</th><th>' + tr('产品线', 'Product Line') + '</th><th>' + tr('状态', 'Status') + '</th><th>' + tr('计划开始时间', 'Planned Start') + '</th><th>' + tr('备案原因', 'Filing Reason') + '</th><th>' + tr('备注', 'Notes') + '</th></tr></thead><tbody>' + explained.map(row => '<tr class="' + (isRecentPlanStart(row.planStart) ? 'df-recent-row' : '') + '"><td>' + escapeHtml(row.taskNo) + '</td><td>' + escapeHtml(row.operator) + '</td><td>' + escapeHtml(row.bu) + '</td><td>' + escapeHtml(row.customer) + '</td><td>' + escapeHtml(row.family) + '</td><td>' + escapeHtml(row.product) + '</td><td>' + escapeHtml(row.productLine) + '</td><td>' + escapeHtml(row.status) + '</td><td>' + escapeHtml(row.planStart) + '</td><td class="df-wrap-cell">' + escapeHtml(row.reason) + '</td><td class="df-wrap-cell">' + escapeHtml(row.remark) + '</td></tr>').join('') + '</tbody></table></div></section>';
        ...
        function exportWorkbook() {
            ...
            const filingDetail = [[cell(tr('备案原因与备注明细', 'Filing Reasons & Notes'), 1)], [cell(selected.year + '-' + selected.label + ' · ' + scopeLabel, 3)], [cell(tr('任务单号', 'Task No.'), 2), cell(tr('操作人', 'Operator'), 2), cell('BU', 2), cell(tr('客户名', 'Customer'), 2), cell(tr('产品族', 'Product Family'), 2), cell(tr('产品', 'Product'), 2), cell(tr('产品线', 'Product Line'), 2), cell(tr('状态', 'Status'), 2), cell(tr('计划开始时间', 'Planned Start'), 2), cell(tr('备案原因', 'Filing Reason'), 2), cell(tr('备注', 'Notes'), 2)]];
            sortByPlanStart(filing.detail).forEach(row => filingDetail.push([row.taskNo, row.operator, row.bu, row.customer, row.family, row.product, row.productLine, row.status, row.planStart, row.reason, row.remark].map(value => cell(value, isRecentPlanStart(row.planStart) ? 5 : 3))));
            ...
            // dashboardData.detail.forEach is intentionally replaced with a planned-start ordered traversal.
            sortByPlanStart(dashboardData.detail).forEach(row => raw.push(keys.map(key => { const value = smartValue(row[key], key); return cell(value, isRecentPlanStart(planStartValue(row)) ? 5 : 3); })));
            ...
            files['xl/styles.xml'] = files['xl/styles.xml'].replace('<cellXfs count="5">', '<cellXfs count="6">').replace('</cellXfs>', '<xf numFmtId="0" fontId="0" fillId="3" borderId="1" xfId="0" applyFill="1" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf></cellXfs>');
        ...
        function detailTable(rows) {
            ...
            const orderedRows = sortByPlanStart(rows); const keys = detailColumns(orderedRows); const pageCount = Math.max(1, Math.ceil(orderedRows.length / detailPageSize)); detailPage = Math.min(pageCount, Math.max(1, detailPage)); const start = (detailPage - 1) * detailPageSize; // pageRows = rows.slice(start, start + detailPageSize) after planned-start sorting
            const pageRows = orderedRows.slice(start, start + detailPageSize);
            return '<section class="df-card"><header><div><strong>' + tr('当月业务比对原始明细', 'Monthly Raw Operation Detail') + '</strong><small>' + tr('按计划开始时间由近到远；最近 7 天内计划行已高亮；全部 ', 'Sorted by planned start time; last-7-day rows highlighted; all ') + keys.length + tr(' 列 · 自动分页展示全部数据 · 原始明细仍保留 ICT和待评分记录', ' columns · all rows paginated · raw ICT and pending-rating rows retained') + '</small></div><span class="df-count">' + fmt(orderedRows.length) + tr(' 条', ' rows') + '</span></header><div class="df-table-wrap"><table><thead><tr>' + keys.map(key => '<th>' + escapeHtml(key) + '</th>').join('') + '</tr></thead><tbody>' + pageRows.map(row => '<tr class="' + (isRecentPlanStart(planStartValue(row)) ? 'df-recent-row' : '') + '">' + keys.map(key => '<td title="' + escapeHtml(smartValue(row[key], key)) + '">' + escapeHtml(smartValue(row[key], key)) + '</td>').join('') + '</tr>').join('') + '</tbody></table></div><div class="df-pager"><span>' + tr('每页', 'Rows') + ' <select data-df-page-size><option value="25"' + (detailPageSize === 25 ? ' selected' : '') + '>25</option><option value="50"' + (detailPageSize === 50 ? ' selected' : '') + '>50</option><option value="100"' + (detailPageSize === 100 ? ' selected' : '') + '>100</option></select></span><button data-df-page="prev"' + (detailPage <= 1 ? ' disabled' : '') + '>' + tr('上一页', 'Previous') + '</button><b>' + detailPage + ' / ' + pageCount + '</b><button data-df-page="next"' + (detailPage >= pageCount ? ' disabled' : '') + '>' + tr('下一页', 'Next') + '</button><span>' + tr('当前 ', 'Showing ') + (start + 1) + '–' + Math.min(orderedRows.length, start + detailPageSize) + ' / ' + orderedRows.length + '</span></div></section>';
        }
        ...
        async function loadDashboard() {
            if (loading) return; if (simulationMode) { status.textContent = tr('模拟浮窗不访问正式接口，请点击“导入 JSON”离线回顾。', 'Simulation does not call live APIs. Import JSON for offline review.'); return; } reviewInfo = null; status.classList.remove('review'); if (typeof reviewBadge !== 'undefined') reviewBadge.classList.remove('active'); loading = true; const token = ++loadToken; refresh.disabled = true; exportButton.disabled = true; if (typeof jsonExportButton !== 'undefined') jsonExportButton.disabled = true; yearSelect.disabled = true; monthSelect.disabled = true; scopeControl.disabled = true; pendingControl.disabled = true; content.innerHTML = '<div class="df-loading">' + tr('正在获取各月公式汇总与明细，并校正剔除口径…', 'Loading monthly summaries and detail to reconcile exclusion scope…') + '</div>';
            try {
                const now = new Date(); const lastMonth = settings.year === now.getFullYear() ? now.getMonth() + 1 : 12; const months = []; const detailsByMonth = {}; let detail = [];
                ...
                for (let month = 1; month <= lastMonth; month++) {
                    ...
                    try { const result = await Promise.all([fetchMonthSummary(settings.year, month), fetchDetail(settings.year, month)]); months.push(applyDetailScope(result[0], result[1])); detailsByMonth[String(month)] = result[1]; if (month === settings.month) detail = result[1]; }
                    catch (error) { throw new Error(settings.year + '-' + String(month).padStart(2, '0') + ' · ' + (error.message || error)); }
                }
                if (destroyed || token !== loadToken) return;
                dashboardData = { months, detail, detailsByMonth }; capturedAt = new Date().toISOString(); detailPage = 1; status.textContent = tr('范围：', 'Scope: ') + CONFIG.region + ' / ' + CONFIG.office + ' / High·Fatal·Medium · ' + (settings.excludeIct ? tr('已剔除 ICT服务与软件', 'ICT Services & Software excluded') + ' · ' : '') + (settings.excludePendingScore ? tr('已剔除待评分任务', 'Pending-rating tasks excluded') + ' · ' : '') + tr('更新于 ', 'Updated ') + new Date().toLocaleString(); renderDashboard(); saveSettings();
            } catch (error) {
            ...
            } finally { loading = false; refresh.disabled = false; exportButton.disabled = !dashboardData; if (typeof jsonExportButton !== 'undefined') jsonExportButton.disabled = !dashboardData; yearSelect.disabled = false; monthSelect.disabled = false; scopeControl.disabled = false; pendingControl.disabled = false; }
        }
        ...
        style.textContent += '.df-json{border-color:rgba(251,191,36,.42);background:rgba(120,53,15,.32);color:#fde68a}.df-review-badge{display:none;align-items:center;height:28px;box-sizing:border-box;padding:0 9px;border:1px solid #fb7185;border-radius:999px;background:linear-gradient(135deg,#9f1239,#be123c);color:#fff;font-size:9px;font-weight:950;box-shadow:0 0 16px rgba(244,63,94,.32);white-space:nowrap}.df-review-badge.active{display:inline-flex}.df-status.review{color:#fecdd3;background:rgba(159,18,57,.2);font-weight:800}';
        ...
        function formatSnapshotTime(value) { const date = new Date(value); return Number.isNaN(date.getTime()) ? String(value || '-') : date.toLocaleString(settings.language === 'en' ? 'en-GB' : 'zh-CN', { hour12: false }); }
        function updateReviewUi() {
            const reviewing = Boolean(reviewInfo); reviewBadge.classList.toggle('active', reviewing); status.classList.toggle('review', reviewing);
            reviewBadge.textContent = reviewing ? tr('回顾模式', 'REVIEW MODE') + ' · ' + formatSnapshotTime(reviewInfo.capturedAt) : '';
            if (reviewing && dashboardData) { const rawCount = Object.values(dashboardData.detailsByMonth || {}).reduce((total, rows) => total + (Array.isArray(rows) ? rows.length : 0), 0); status.textContent = tr('回顾数据时间：', 'Snapshot time: ') + formatSnapshotTime(reviewInfo.capturedAt) + tr(' · 导出时间：', ' · Exported: ') + formatSnapshotTime(reviewInfo.exportedAt) + tr(' · 月度汇总 ', ' · Monthly summaries ') + dashboardData.months.length + tr(' 个月 · 全年原始明细 ', ' months · All raw detail ') + rawCount + tr(' 条 · 当前查看 ', ' rows · Viewing ') + settings.year + '-' + String(settings.month).padStart(2, '0') + (simulationMode ? tr(' · 离线模拟，不访问正式接口', ' · Offline simulation; live APIs are disabled') : tr(' · 点击“刷新专题”返回实时模式', ' · Refresh to return to live mode')); }
            jsonExportButton.disabled = !dashboardData || loading;
        }
        function downloadTopicSnapshot() {
            if (!dashboardData) return; const exportedAt = new Date().toISOString();
            const snapshot = { schema: 'uivf12-topic-snapshot', version: 1, platform: 'datafab', exportedAt, capturedAt: reviewInfo && reviewInfo.capturedAt || capturedAt || exportedAt, scope: { region: CONFIG.region, office: CONFIG.office, country: CONFIG.country }, settings: { year: settings.year, month: settings.month, excludeIct: settings.excludeIct, excludePendingScore: settings.excludePendingScore, targets: settings.targets, activeTopic }, data: dashboardData };
            const blob = new Blob([JSON.stringify(snapshot, null, 2)], { type: 'application/json;charset=utf-8' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'DataFab专题全量数据_' + exportedAt.slice(0, 19).replace(/[-:T]/g, '') + '.json'; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(link.href);
        }
        function importTopicSnapshot(file) {
            const reader = new FileReader(); reader.onload = async function () {
                try {
                    const snapshot = JSON.parse(String(reader.result || '')); const data = snapshot && snapshot.data;
                    if (!snapshot || snapshot.schema !== 'uivf12-topic-snapshot' || snapshot.version !== 1 || snapshot.platform !== 'datafab') throw new Error(tr('不是有效的 DataFab 专题全量数据文件', 'Not a valid DataFab topic snapshot'));
                    if (!data || !Array.isArray(data.months) || !Array.isArray(data.detail)) throw new Error(tr('专题数据结构不完整', 'Snapshot data is incomplete'));
                    const imported = snapshot.settings || {}; const year = Math.floor(Number(imported.year)); const month = Math.floor(Number(imported.month));
                    if (Number.isFinite(year)) settings.year = year; if (month >= 1 && month <= 12) settings.month = month;
                    settings.excludeIct = imported.excludeIct !== false; settings.excludePendingScore = imported.excludePendingScore !== false;
                    if (imported.targets && Number.isFinite(Number(imported.targets.returnRate))) settings.targets.returnRate = Math.min(100, Math.max(0, Number(imported.targets.returnRate)));
                    if (imported.targets && Number.isFinite(Number(imported.targets.recordRate))) settings.targets.recordRate = Math.min(100, Math.max(0, Number(imported.targets.recordRate)));
                    activeTopic = imported.activeTopic === 'record' ? 'record' : 'return'; data.detailsByMonth = data.detailsByMonth && typeof data.detailsByMonth === 'object' ? data.detailsByMonth : { [String(settings.month)]: data.detail }; data.detail = Array.isArray(data.detailsByMonth[String(settings.month)]) ? data.detailsByMonth[String(settings.month)] : data.detail; dashboardData = data; capturedAt = snapshot.capturedAt || snapshot.exportedAt; reviewInfo = { capturedAt, exportedAt: snapshot.exportedAt || capturedAt }; detailPage = 1;
                    if (![...yearSelect.options].some(option => Number(option.value) === settings.year)) yearSelect.insertAdjacentHTML('afterbegin', '<option value="' + settings.year + '">' + settings.year + '</option>');
                    yearSelect.value = String(settings.year); monthSelect.value = String(settings.month); scopeControl.checked = settings.excludeIct; pendingControl.checked = settings.excludePendingScore;
                    mode.querySelectorAll('.df-target').forEach(input => { input.value = settings.targets[input.dataset.target]; }); mode.querySelectorAll('[data-df-topic]').forEach(tab => tab.classList.toggle('active', tab.dataset.dfTopic === activeTopic));
                    renderDashboard(); updateReviewUi(); exportButton.disabled = false;
                    if (options && typeof options.onSnapshotImported === 'function') {
                        try {
                            const saved = await options.onSnapshotImported(snapshot);
                            status.textContent += tr(' · 已归档到服务器', ' · Archived on server') + (saved && saved.created === false ? tr('（已存在）', ' (already existed)') : '');
                        } catch (saveError) {
                            status.textContent += tr(' · 服务器归档失败：', ' · Server archive failed: ') + (saveError && saveError.message || String(saveError));
                        }
                    }
                } catch (error) { status.textContent = tr('导入失败：', 'Import failed: ') + (error && error.message || String(error)); status.classList.remove('review'); }
        ...
        function setMode(next) {
            ...
            if (active && !dashboardData && !loading && !simulationMode) loadDashboard();
        }
        ...
        monthSelect.addEventListener('change', () => { settings.month = Number(monthSelect.value); if (reviewInfo && dashboardData && dashboardData.detailsByMonth && Array.isArray(dashboardData.detailsByMonth[String(settings.month)])) { dashboardData.detail = dashboardData.detailsByMonth[String(settings.month)]; detailPage = 1; saveSettings(); renderDashboard(); updateReviewUi(); return; } dashboardData = null; saveSettings(); loadDashboard(); });
        ...
        mode.querySelector('.df-language').addEventListener('click', event => {
            ...
            event.currentTarget.textContent = settings.language === 'en' ? '中' : 'EN'; refresh.textContent = tr('刷新专题', 'Refresh'); exportButton.textContent = tr('导出 Excel', 'Export Excel'); jsonExportButton.textContent = tr('导出 JSON', 'Export JSON'); jsonImportButton.textContent = tr('导入 JSON', 'Import JSON');
            ...
            button.textContent = active ? tr('返回 CSV', 'Back to CSV') : tr('DataFab 专题', 'DataFab Insights'); renderDashboard(); updateReviewUi();
        });
        exportButton.addEventListener('click', exportWorkbook);
        jsonExportButton.addEventListener('click', downloadTopicSnapshot);
        jsonImportButton.addEventListener('click', () => jsonFileInput.click());
        jsonFileInput.addEventListener('change', () => { if (jsonFileInput.files && jsonFileInput.files[0]) importTopicSnapshot(jsonFileInput.files[0]); });
        refresh.addEventListener('click', () => { if (simulationMode) { status.textContent = tr('模拟浮窗不访问正式接口，请导入 JSON 回顾历史数据。', 'Simulation does not call live APIs. Import JSON to review historical data.'); return; } dashboardData = null; loadDashboard(); });
    ...
    window.UIVDataFabAnalysis = {
        ...
        installForSimulation(root, options) { return installDataFabAnalysisRuntime(root, Object.assign({}, options, { simulation: true })); }
    };
})();
