...
(function () {
    ...
    function installDataFabAnalysisRuntime(root, options) {
        ...
        function analyzeFilingRows(rows) {
            ...
            (rows || []).forEach(source => {
                const row = filingRow(source); if (settings.excludeIct && isExcludedIctLine(row.productLine)) return;
                if (settings.excludePendingScore && isPendingScoreStatus(row.status)) return;
                ...
                if (hasReason) {
                    ...
                    const operator = String(row.operator || tr('未标注', 'Unspecified')).trim(); const operatorItem = operators.get(operator) || { label: operator, count: 0, bus: new Map(), customers: new Map() }; operatorItem.count++;
                    const bu = String(row.bu || tr('未标注', 'Unspecified')).trim(); const customer = String(row.customer || tr('未标注', 'Unspecified')).trim(); operatorItem.bus.set(bu, (operatorItem.bus.get(bu) || 0) + 1); operatorItem.customers.set(customer, (operatorItem.customers.get(customer) || 0) + 1); operators.set(operator, operatorItem);
                }
            });
            ...
            const topLabels = map => Array.from(map.entries()).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 3).map(entry => entry[0]).join(' / ');
            const operatorRanking = Array.from(operators.values()).map(item => ({ label: item.label, count: item.count, share: filed ? item.count / filed * 100 : 0, bu: topLabels(item.bus), customer: topLabels(item.customers) })).sort((a, b) => b.count - a.count || a.label.localeCompare(b.label));
        ...
        function filingOperatorCard(items) {
            ...
            return '<section class="df-card df-wide"><header><div><strong>' + tr('高频备案操作人', 'Frequent Filing Operators') + '</strong><small>' + tr('仅统计已填写备案原因的记录；BU 和客户按该操作人备案频次展示前 3 项', 'Only rows with a filing reason; top 3 BUs and customers by operator filing frequency') + '</small></div></header><div class="df-operator-list">' + (visible.length ? visible.map(item => '<div class="df-operator-row"><b title="' + escapeHtml(item.label) + '">' + escapeHtml(item.label) + '</b><span><em>BU</em>' + escapeHtml(item.bu) + '</span><span><em>' + tr('客户', 'Customer') + '</em>' + escapeHtml(item.customer) + '</span><div><i style="width:' + (item.count / max * 100) + '%"></i></div><strong>' + fmt(item.count) + '</strong><small>' + fmtRate(item.share) + '</small></div>').join('') : '<div class="df-empty">' + tr('暂无可识别的备案操作人', 'No filing operators identified') + '</div>') + '</div></section>';
        }
        ...
        function exportWorkbook() {
            ...
            const filing = analyzeFilingRows(dashboardData.detail); const filingRows = [[cell(tr('备案多维分析', 'Multidimensional Filing Analysis'), 1)], [cell(selected.year + '-' + selected.label + ' · ' + scopeLabel + ' · ' + tr('备案口径：备案原因非空；全部维度按备案数量降序', 'Filed means filing reason is populated; all dimensions sorted by filing count'), 3)], [cell(tr('分析维度', 'Dimension'), 2), cell(tr('维度值', 'Value'), 2), cell(tr('归属BU', 'Related BU'), 2), cell(tr('客户名', 'Customer'), 2), cell(tr('基数', 'Base'), 2), cell(tr('备案数量', 'Filing Count'), 2), cell(tr('未备案', 'Missing'), 2), cell(tr('备案占比/覆盖率', 'Filing Share/Coverage'), 2), cell(tr('有备注', 'With Notes'), 2)]];
            [['BU', filing.dimensions.bu], [tr('客户', 'Customer'), filing.dimensions.customer], [tr('产品族', 'Product Family'), filing.dimensions.family], [tr('产品', 'Product'), filing.dimensions.product]].forEach(group => group[1].forEach(item => filingRows.push([cell(group[0], 3), cell(item.label, 3), cell('', 3), cell('', 3), cell(item.total, 3), cell(item.filed, 3), cell(item.pending, 3), cell(item.rate / 100, 4), cell(item.noted, 3)])));
            filing.operators.forEach(item => filingRows.push([cell(tr('高频操作人', 'Frequent Operator'), 3), cell(item.label, 3), cell(item.bu, 3), cell(item.customer, 3), cell(filing.filed, 3), cell(item.count, 3), cell('', 3), cell(item.share / 100, 4), cell('', 3)]));
            const filingDetail = [[cell(tr('备案原因与备注明细', 'Filing Reasons & Notes'), 1)], [cell(selected.year + '-' + selected.label + ' · ' + scopeLabel, 3)], [cell(tr('操作人', 'Operator'), 2), cell('BU', 2), cell(tr('客户名', 'Customer'), 2), cell(tr('产品族', 'Product Family'), 2), cell(tr('产品', 'Product'), 2), cell(tr('产品线', 'Product Line'), 2), cell(tr('状态', 'Status'), 2), cell(tr('备案原因', 'Filing Reason'), 2), cell(tr('备注', 'Notes'), 2)]];
            filing.detail.forEach(row => filingDetail.push([row.operator, row.bu, row.customer, row.family, row.product, row.productLine, row.status, row.reason, row.remark].map(value => cell(value, 3))));
