...
- API 路由位于 `backend/routes`（共 15 个路由文件）。
...
tools-platform/
  ...
  frontend/
    ...
    pages/                      # 各业务页面
      ...
      terms.html
    js/
      shared/                   # 公共组件（navbar、toast、ai-assistant 等）
      sla/
      report/
      uivf12/
      ...
    css/
    assets/
    googlea8435fd020ce60ab.html # Google Search Console 验证文件
    add_cover_watermark.py      # PDF 封面水印工具（月报/PR 审计导出用）
    fix_pdf_groups.py           # PDF 分组修复脚本
    update_css.py               # CSS 批量更新工具
  data/
    report.db                   # 报表看板入库数据
    requirements.db             # 需求广场数据
    images/                     # 报表截图和 Excel 导出
  outputs/
```

> `frontend/` 根目录下的 `.py` 脚本是辅助工具，不在运行时执行，需手动运行。`googlea8435fd020ce60ab.html` 是 Google Search Console 所有权验证文件，可忽略。
